...
class HGD(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = nn.Sequential(
                conv3x3_hgd_cb(inplanes, planes, stride, halving=0),
                norm_layer(planes),
                nn.ReLU(inplace=True),
                BasicConv3d(planes, planes, kernel_size=(3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0)),
                norm_layer(planes),
...
class HiH_OUMVLP(BaseModel):
   ...
   def build_network(self, model_cfg):
       ...
       self.layer2 = nn.ModuleList(
           [_ for _ in self.make_layer(HGD, 1 * C, 2 * C, stride=1, stage=2, enhance=False, blocks_num=B[1])]
       )
       ...
       self.layer3 = nn.ModuleList(
           [_ for _ in self.make_layer(HGD, 2 * C, 4 * C, stride=1, stage=3, enhance=False, blocks_num=B[2])]
       )

       self.layer4 = nn.ModuleList(
           [_ for _ in self.make_layer(HGD, 4 * C, 4 * C, stride=1, stage=3, enhance=False, blocks_num=B[3])]
       )
